...
import { lookupUsdaMatchWithMetadata } from '../services/nutrition/usda/usdaNutritionLookupProvider';
  ...
  if (!process.env.USDA_FDC_API_KEY) {
    console.error(
      'USDA_FDC_API_KEY is not set.\n' +
        'Copy .env.example to .env.local and fill in a real key from https://fdc.nal.usda.gov/api-key-signup.html, then re-run.',
    );
    process.exitCode = 1;
    return;
  }
