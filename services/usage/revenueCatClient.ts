...
export interface RevenueCatCustomer {
  /**
   * Entitlement IDs currently active for this customer. RevenueCat has
   * already resolved expiry and billing-retry grace periods server-side
   * — membership in this list means "grants access right now", full
   * stop, no further date math needed by the caller.
   */
...
export async function fetchRevenueCatActiveEntitlements(customerId: string): Promise<RevenueCatCustomer | null> {
  ...
  const activeEntitlementIds = (data.items ?? [])
