...
import { getVisionProvider } from '../src/services/ai/visionProvider';
import { getNutritionLookupProvider } from '../src/services/nutrition/nutritionLookup';
import { runAnalyzePipeline } from '../src/services/analysis/analyzePipeline';
import { calculateFoodItemTotals } from '../src/services/nutrition/calculate';
import { ApiRouteError } from '../src/lib/apiResponse';
import type { FoodMatch } from '../src/types/nutrition';
...
async function main() {
  ...
  if (missing.length > 0) {
    ...
    console.error('Add them to .env (copy from .env.example) before running this script.');
    process.exitCode = 1;
    return;
  }
