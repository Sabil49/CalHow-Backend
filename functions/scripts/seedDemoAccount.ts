 ...
 * Uses Application Default Credentials (see src/lib/firebaseAdmin.ts) —
 * run `gcloud auth application-default login` once, with a Google account
 * that has at least Editor access on the `calhow-712c0` Firebase project,
 * before running this script. No service-account key file needed.
 *
 * Deliberately self-contained (no import from calhow-mobile/types) — this
 * file is typechecked as part of this repo's own build, which runs in
 * environments where the sibling calhow-mobile repo isn't checked out.
 */
