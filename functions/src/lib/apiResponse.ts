...
/**
 * Fixed vocabulary of error codes used across every function in this
 * backend. Keeping this as a closed set (rather than ad-hoc strings per
 * handler) means the mobile app's error handling — and any logging/alerting
...
/**
 * Thrown by service/handler code to signal a specific, structured API
 * error. Handlers should catch this (see lib/auth.ts's withAuth) and
 * translate it via `jsonError`, rather than constructing ad-hoc error
 * responses inline.
 */
...
export function jsonError(res: Response, code: ApiErrorCode, message: string, statusOverride?: number): void {
