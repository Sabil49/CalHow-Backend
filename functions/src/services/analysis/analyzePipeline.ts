 ...
 * extracted from the handler specifically so it's testable with a
 * mocked AI provider and a mocked nutrition lookup provider — no Firebase
 * Admin auth, no real network calls, no Express request/response
 * plumbing required to exercise this logic. See
 * services/analysis/__tests__/analyzePipeline.test.ts.
 *
 * The handler (meals/analyze.ts) is left with just: auth, request
 * validation, usage check, calling this, persisting the result, and
