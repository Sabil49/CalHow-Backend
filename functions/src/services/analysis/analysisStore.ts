 ...
 * in-memory session (Cloud Functions invocations are stateless).
 *
 ...
 * expired documents are deleted opportunistically when encountered. A
 * scheduled Cloud Function to sweep expired docs proactively is possible
 * now (Blaze plan), but not implemented here; lazy enforcement is still
 * correct on its own, just leaves expired docs lingering until next read.
 */
