...
export interface ScanQuotaStatus {
  ...
  reason?: string;
  /**
   * Set only when a free-tier scan was actually consumed: the UTC day key it
   * was counted against, so `refundScan` gives it back to the SAME day even
   * if the request straddled midnight. Server-internal — never sent to the client.
   */
  consumedDateKey?: string;
}
...
export interface ScanQuotaStore {
  ...
  /** Atomic "decrement (uid, dateKey)'s count by one, never below zero" — see `refundScan`. */
  decrement(uid: string, dateKey: string): Promise<void>;
}
...
export function createFirestoreScanQuotaStore(): ScanQuotaStore {
  return {
    async incrementIfUnder(uid, dateKey, limit) {
      ...
      const { FieldValue } = getFirestoreValues();
    ...
    async decrement(uid, dateKey) {
      ...
      await db.runTransaction(async (tx) => {
        ...
        const current = snap.exists ? ((snap.data()?.scansUsed as number | undefined) ?? 0) : 0;
        if (current <= 0) return;
        tx.set(ref, { scansUsed: current - 1, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
      });
    },
  };
}
...
/**
 * NOTE: the "no refund" policy described below has since been replaced —
 * scans that fail on our side are now given back via `refundScan` (see
 * its doc comment for why that's concurrency-safe). The rest still holds.
 *
...
export async function consumeScanIfAllowed(uid: string, deps: ScanQuotaDeps = defaultDeps): Promise<ScanQuotaStatus> {
  ...
  return {
    ...
    dailyScanLimit: DAILY_FREE_SCAN_LIMIT,
    consumedDateKey: dateKey,
  };
}

/**
 * Gives back a scan that `consumeScanIfAllowed` counted, for a request that
 * then failed on OUR side (AI provider / USDA outage or timeout, internal
 * error) — so a free user isn't locked out for the day by failures they
 * didn't cause (e.g. cold-start timeouts burning all their free scans).
 * The analyze handler decides which failures qualify; see meals/analyze.ts.
 *
 * Safe under concurrency: it's its own atomic transaction that only ever
 * decrements, never below zero, and only for a scan this same request
 * consumed (`consumedDateKey` is absent for Pro or denied requests, which
 * makes this a no-op). Interleaving with a concurrent consume just means
 * the counter ends where it would have if the failed scan never happened.
 * Never throws — a refund that fails is logged, not surfaced over the
 * original error.
 */
export async function refundScan(uid: string, status: ScanQuotaStatus, deps: Pick<ScanQuotaDeps, 'store'> = defaultDeps): Promise<void> {
  if (!status.consumedDateKey) return;
  try {
    await deps.store.decrement(uid, status.consumedDateKey);
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error('[scanLimit] refund failed', err);
  }
}
